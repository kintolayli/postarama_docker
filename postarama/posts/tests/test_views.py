...
class StaticURLTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_group = Group.objects.create(
            title="Кошки", slug="cats", description="Посты про кошек"
        )

        cls.key = make_template_fragment_key("index_page")

        cls.index_url = reverse("index")
        cls.group_url = reverse("group_posts", kwargs={"slug": cls.test_group.slug})
        cls.new_post_url = reverse("new_post")
        cls.profile_url = reverse("profile", kwargs={"username": cls.user_1.username})

    def test_authorized_user_new_post(self):
        current_posts_count = Post.objects.count()
        response = self.authorized_client.post(
            self.new_post_url, {"text": "Это текст публикации"}, follow=True
        )
    ...
    def test_unauthorized_user_edit_post(self):
        ...
        edit_post_url = reverse(
            "post_edit",
            kwargs={"username": self.user_1.username, "post_id": new_post.id},
        )
        ...
        expected_url = f'{reverse("login")}?next={edit_post_url}'

        self.assertRedirects(
            edit_post, expected_url, status_code=302, target_status_code=200
        )
    ...
    def test_authorized_user_edit_post_and_post_changed_everywhere(self):
        ...
        new_group = Group.objects.create(
            title="Собаки", slug="dogs", description="Посты про собак"
        )
        ...
        edit_post_url = reverse(
            "post_edit",
            kwargs={"username": self.user_1.username, "post_id": post.id},
        )
        post_url = reverse(
            "post",
            kwargs={"username": self.user_1.username, "post_id": post.id},
        ...
        urls = [self.index_url, self.profile_url, post_url, new_group_url]
    ...
    def test_follow_and_unfollow(self):
        followers_count_user_2 = Follow.objects.filter(user=self.user_2).count()

        follow_to_user_1 = reverse(
            "profile_follow", kwargs={"username": self.user_1.username}
        )
        unfollow_to_user_1 = reverse(
            "profile_unfollow", kwargs={"username": self.user_1.username}
        )
        ...
        self.assertNotEqual(followers_count_user_2, followers_count_user_2_after_follow)
        ...
        self.assertEqual(followers_count_user_2, followers_count_user_2_after_unfollow)
    ...
    def test_view_post_if_user_is_subscribed_to_another_users(self):
        ...
        follow_to_user_1_url = reverse(
            "profile_follow", kwargs={"username": self.user_1.username}
        )
        unfollow_to_user_1_url = reverse(
            "profile_unfollow", kwargs={"username": self.user_1.username}
        )
    ...
    def test_authorized_user_can_comment_delete(self):
        ...
        self.assertNotEqual(comments_count_before_delete, comments_count_after_delete)
