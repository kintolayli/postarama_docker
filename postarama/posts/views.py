...
from posts.models import Comment, Follow, Group, GroupFollow, Post
...
FOLLOW_SUCCESS_USER = (
    "Готово! Теперь ты будешь получать обновления " "от этого пользователя."
)
FOLLOW_SUCCESS_GROUP = "Готово! Теперь ты будешь получать " "обновления от этой группы."
...
def sort_by_likes():
    ...
    return (
        ...
        .annotate(posts_most_likes=Sum("likes__vote"))
        .order_by("posts_most_likes")
...
def index(request, tag_slug=None):
    posts = Post.objects.published().order_by("-publish").select_related("author")
    ...
    return render(
        request,
        "index.html",
        {"page": page, "paginator": paginator, "index": True, "tag": tag},
    )
...
def group_posts(request, slug):
    ...
    try:
        group_follower = get_object_or_404(GroupFollow, user=request.user, group=group)
...
def post_view(request, username, post_id):
    ...
    comments = (
        ...
        .annotate(comments_most_likes=Sum("likes__vote"))
        .order_by("comments_most_likes")
        .select_related("author")
    )
    ...
    similar_posts = (
        Post.objects.published().filter(tags__in=post_tags_ids).exclude(id=post.id)
    )
...
def edit_comment(request, username, post_id, comment_id):
    ...
    comment = get_object_or_404(
        Comment, author=request.user, post=post_id, pk=comment_id
    )
...
def follow_index(request):
    ...
    group_posts_list = (
        Post.objects.published()
        .filter(group__followers__user=request.user)
        .select_related("author")
    )

    result_list = author_posts_list.union(group_posts_list, all=False).order_by(
        "-publish"
    )
...
def group_unfollow(request, slug):
    group = get_object_or_404(Group, slug=slug)
    subscription = get_object_or_404(GroupFollow, user=request.user, group=group)
    subscription.delete()
