...
class TestComment:
    def test_comment_model(self):
        ...
        assert (
            created_field is not None
        ), "Добавьте дату и время проведения события `created` модели `Comment`"
        assert (
            type(created_field) == fields.DateTimeField
        ), "Свойство `created` модели `Comment` должно быть датой и время `DateTimeField`"
        assert (
            created_field.auto_now_add
        ), "Свойство `created` модели `Comment` должно быть `auto_now_add`"
    ...
    def test_comment_add_view(self, client, post):
        try:
            ...
            assert (
                False
            ), f"""Страница `/<username>/<post_id>/comment/` работает неправильно. Ошибка: `{e}`"""
        if (
        ...
        if not (
            ...
            assert (
                False
            ), "Проверьте, что не авторизованного пользователя `/<username>/<post_id>/comment/` отправляете на страницу авторизации"
