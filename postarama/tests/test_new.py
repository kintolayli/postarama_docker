...
class TestNewView:
    ...
    def test_new_view_get(self, user_client):
        ...
        ), "Страница `/new/` не найдена, проверьте этот адрес в *urls.py*"
        assert (
            "form" in response.context
        ), "Проверьте, что передали форму `form` в контекст страницы `/new/`"
        assert (
            len(response.context["form"].fields) == 3
        ), "Проверьте, что в форме `form` на страницу `/new/` 3 поля"
        assert (
    ...
    def test_new_view_post(self, user_client, user, group):
        ...
        assert (
            post is not None
        ), "Проверьте, что вы сохранили новый пост при отправки формы на странице `/new/`"
        assert (
            response.url == "/"
