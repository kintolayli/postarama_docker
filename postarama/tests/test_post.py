...
class TestPostView:
    ...
    def test_post_view_get(self, client, post_with_group):
        try:
            ...
            assert (
                False
            ), f"""Страница `/<username>/<post_id>/` работает неправильно. Ошибка: `{e}`"""
        ...
        assert (
            type(comment_form_context.fields["text"]) == forms.fields.CharField
...
class TestPostEditView:
    ...
    def test_post_edit_view_get(self, client, post_with_group):
        try:
            ...
            assert (
                False
            ), f"""Страница `/<username>/<post_id>/edit/` работает неправильно. Ошибка: `{e}`"""
